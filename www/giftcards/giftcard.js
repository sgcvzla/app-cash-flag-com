...
function inicio() {
	...
	xmlhttp.open("GET", "https://app.cash-flag.com/php/buscaprepago.php?idsocio="+idsocio, true);
	xmlhttp.send();
}
...
function pagoenlinea(divisa) {
	...
	xmlhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			if (respuesta.exito == 'SI') {
				...
				window.open("https://app.cash-flag.com/php/formapagoenlinea.php?card="+respuesta.card+"&monto="+monto+"&ruta=prepago","_blank",propiedades);
			} else {
	...
	if (divisa=="bs") {
		xmlhttp.open("POST", "https://app.cash-flag.com/php/pagoenlinea.php", true);
	// } else {
	// 	xmlhttp.open("POST", "https://app.cash-flag.com/php/reportapago.php", true);
	}
	xmlhttp.send(datos);
}
